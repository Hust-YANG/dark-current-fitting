...
def solve_implicit(V, J01, A1, J02, A2, R_S, R_SH, k, m,
    ...
    for i in range(max_iter):
        ...
        if diff < tol:
            if verbose and i > 3: print(f"    Converged in {i+1} iterations")
...
def _parse_raw_data(fp):
    if not os.path.exists(fp): print(f"Error: file not found — {fp}"); sys.exit(1)
    ...
    with open(fp, 'r', encoding=enc) as f:
        for ln in f:
            ...
            try:
                ...
                else: flat_v.append(float(parts[0])); flat_i.append(float(parts[1]))
...
def load_data(fp, area=None, auto_dark=True, manual_sweep=None, max_points=None):
    ...
    elif auto_dark and len(sweeps) > 1: use = _identify_dark(sweeps, fp)
    else: use = list(sweeps.keys())
    all_v, all_i = [], []
    for sid in sorted(use): all_v.extend(sweeps[sid]['V']); all_i.extend(sweeps[sid]['I'])
    if max_points: all_v = all_v[:max_points]; all_i = all_i[:max_points]
...
def fit_dark_current(V, J, model_type='dual', T=300):
    ...
    if model_type == 'dual': Jd, V_int, conv = solve_implicit(V, *best, verbose=True)
    else:
...
def plot_fitting(V, J, fit, ax):
    ...
    ax.semilogy(V_plot, np.abs(JTAT), ':', lw=1.0, color=COLOR_PALETTE['j_tat'],
                label='$J_{\\mathrm{TAT}}$', zorder=3)
...
def export_params(fit, output_dir, label='fitting'):
    ...
    if model_type == 'dual':
        rows.extend([
            ...
            ('k (TAT coef)', 'k', '-', f'{popt[6]:.4e}'),
            ('m (TAT barrier)', 'm', 'V', f'{popt[7]:.4f}'),
        ...
        rows.extend([
            ...
            ('k (TAT coef)', 'k', '-', f'{popt[4]:.4e}'),
            ('m (TAT barrier)', 'm', 'V', f'{popt[5]:.4f}'),
            ('R²', 'R²', '-', f'{fit["r_squared"]:.6f}'),
        ])
...
def generate_word_report(fit, V, J, output_dir, T=300, area=None, label='sample'):
    ...
    if os.path.exists(img_path):
        ...
        r = cap.add_run('Figure 1. Dark current J-V characteristics and fitting results.')
    ...
    idx += 1
    doc.add_heading(f'{idx}. Trap-Assisted Tunneling (J_TAT)', 2)
    doc.add_paragraph(f'J_TAT = k·V·exp(m/V_int). k={k_val:.4e}, m={m_val:.4f} V. m<0 → TAT grows with reverse bias.')
    ...
    tdata += [('R_S (Series resistance)', f'{R_S:.2f}', 'Ω·cm²'),
              ...
              ('k (TAT coefficient)', f'{k_val:.4e}', '-'),
              ('m (TAT barrier)', f'{m_val:.4f}', 'V'),
    ...
    doc.add_paragraph(
        ...
        + f' Optimization: (1) suppress J_TAT (k={k_val:.2e}); (2) minimize R_S ({R_S:.1f} Ω·cm²);'
        + (' (3) passivate G-R centers to reduce J₀₂.' if model_type == 'dual' else ''))
...
def run_dark_current_fitting(data_file, output_dir=None, model='dual',
    ...
    all_results = []; base_name = os.path.splitext(os.path.basename(data_file))[0]

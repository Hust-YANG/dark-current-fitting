...
Model (Dual-Diode, 8 params):
  ...
  where V_int = V - J_D·R_S  (隐式方程, Newton/Picard 迭代求解)
...
T_DEFAULT = 300
CM_PER_INCH = 2.54
MAX_EXP_ARG = 100.0   # clip exponential arguments to prevent overflow
...
# ═══════════════════════════════════════════════════════════════════════════════
# 隐式方程求解 (Implicit Equation Solver)
# ═══════════════════════════════════════════════════════════════════════════════

def _safe_power(V, m):
    """k·V^m with sign protection for negative V and non-integer m."""
    abs_v = np.abs(V, out=np.zeros_like(V) + 1e-30)
    return np.sign(V) * abs_v ** m
...
def _rhs_total(V_int, J01, A1, J02, A2, R_SH, k, m):
    """Right-hand side of the implicit equation (sum of all components)."""
    Jd, Jr, Jo, Jt = _compute_components(V_int, J01, A1, J02, A2, R_SH, k, m)
    return Jd + Jr + Jo + Jt
...
def solve_implicit(V, J01, A1, J02, A2, R_S, R_SH, k, m,
    ...
    Parameters
    ----------
    V : ndarray — applied voltage (V_fit convention: V>0 forward)
    All other params: scalar model parameters
    max_iter : int — max fixed-point iterations
    tol : float — convergence tolerance

    Returns
    -------
    J_D : ndarray — converged total dark current density
    V_int : ndarray — internal junction voltage = V - J_D·R_S
    converged : bool
    """
    Jd = np.zeros_like(V)
    damping = 0.4  # damped fixed-point for stability with large R_S
...
# ═══════════════════════════════════════════════════════════════════════════════
# 模型函数 (Model Functions for curve_fit)
# ═══════════════════════════════════════════════════════════════════════════════
...
def dark_current_single(V, J01, A1, R_S, R_SH, k, m):
    """Single-diode dark current model (J02=0)."""
...
def _parse_raw_data(fp):
    """Parse 3-column data file (Index, V, I) with sweep notation like '1/1'."""
...
def fit_dark_current(V, J, model_type='dual', T=300):
    ...
    if model_type == 'single':
        model_func = dark_current_single
        bounds = BOUNDS_SINGLE
        guesses = GUESSES_SINGLE
        param_names = ['J01', 'A1', 'R_S', 'R_SH', 'k', 'm']
    else:
        model_func = dark_current_dual
        bounds = BOUNDS_DUAL
        guesses = GUESSES_DUAL
    ...
    if model_type == 'dual':
        n2 = 1.0 / (best[3] * Vt) if best[3] > 0 else None
    else:
        n2 = None

    pstr = " ".join(f"{best[j]:.4e}" for j in range(len(best)))
    print(f"  => Best: {pstr} R²={r2:.6f} n1={n1:.3f}" +
...
def configure_plot_style(width_cm=8.5):
    """Configure matplotlib for journal-ready plots."""
    from matplotlib.font_manager import FontProperties, findfont
    cjk_candidates = ['STSong', 'SimSun', 'SimHei', 'Microsoft YaHei', 'Arial Unicode MS']
    cjk_avail = [f for f in cjk_candidates
                 if FontProperties(family=f) and
...
def plot_fitting(V, J, fit, ax):
    """Draw semilogy fitting plot with component decomposition."""
    popt = fit['popt']
    ...
    if model_type == 'dual':
        ...
        J01, A1, R_S, R_SH, k_val, m_val = popt
        J02, A2 = 0.0, 19.3
    ...
    if model_type == 'dual':
        ax.semilogy(V_plot, np.abs(Jrec), '-.', lw=1.0,
                    color=COLOR_PALETTE['j_rec'], label='$J_{\\mathrm{rec}}$', zorder=3)
    ax.semilogy(V_plot, np.abs(Joh), '-.', lw=1.0,
                color=COLOR_PALETTE['j_ohm'], label='$J_{\\mathrm{Ohm}}$', zorder=3)
...
def save_figure_formats(fig, basepath, hd=False):
    """Save figure in SVG, PDF, PNG (300dpi), and optionally 600dpi PNG."""
    saved = []
    for ext, dpi in [('.svg', None), ('.pdf', None), ('.png', 300)]:
        kw = {'format': ext[1:], 'bbox_inches': 'tight',
    ...
    if hd:
        ...
        fig.savefig(fp_hd, dpi=600, bbox_inches='tight',
                    facecolor='white', edgecolor='none')
        saved.append(fp_hd)
    return saved
...
def export_component_data(V, J, fit, output_dir, label='sample'):
    """Export component data to tab-separated .txt file."""
    popt = fit['popt']
    model_type = fit['model_type']
    ...
    Jdiff, Jrec, Joh, Jtun = _compute_components(Vint, J01, A1, J02, A2, R_SH, k_val, m_val)
...
def export_params(fit, output_dir, label='fitting'):
    """Export fitting parameters to CSV."""
    popt = fit['popt']
    model_type = fit['model_type']
    ...
    if model_type == 'dual':
        rows.extend([
            ...
            ('m (tunneling exponent)', 'm', '-', f'{popt[7]:.4f}'),
        ...
        rows.extend([
            ...
            ('m (tunneling exponent)', 'm', '-', f'{popt[5]:.4f}'),
            ('R²', 'R²', '-', f'{fit["r_squared"]:.6f}'),
        ])
    ...
    print(f"\n{'='*50}")
    print(f"  {model_type.upper()}-DIODE FITTING PARAMETERS")
    print(f"{'='*50}")
...
def generate_word_report(fit, V, J, output_dir, T=300, area=None, label='sample'):
    """Generate English academic Word report."""
    try:
        ...
        print("  python-docx not installed — skipping report")
        return None
    ...
    doc.add_paragraph(
        ...
        f'Blue circles: experimental data; orange solid: total fit '
        f'(R²={fit["r_squared"]:.4f}); green dashed: J_diff (diffusion); '
        + ('gold dash-dotted: J_rec (recombination); ' if model_type == 'dual' else '')
        + f'purple dash-dotted: J_Ohm (ohmic); light blue dotted: J_tun (power-law tunneling). '
        f'Excellent agreement is achieved across {V.min():.1f} V to {V.max():.1f} V. '
    ...
    r = eq.add_run('J_D = J₀₁·[exp(A₁·V_int)−1] + J₀₂·[exp(A₂·V_int)−1] '
                   '+ V_int/R_SH + k·V_int^m\n'
                   'V_int = V − J_D·R_S')
    r.font.size = Pt(12)
    r.bold = True
    doc.add_paragraph('')

    eq_desc = doc.add_paragraph()
    eq_desc.alignment = WD_ALIGN_PARAGRAPH.CENTER
    desc_text = ('J_diff (diffusion, n₁≈1) + J_rec (G-R recombination, n₂≈2) '
                 '+ J_Ohm (ohmic shunt) + J_tun (power-law tunneling)'
                 if model_type == 'dual' else
                 'J_diff (diffusion, n₁≈1) + J_Ohm (ohmic shunt) + J_tun (power-law tunneling)')
...
def run_dark_current_fitting(data_file, output_dir=None, model='dual',
    ...
    Vt = thermal_voltage(T)
    print(f"\n{'='*50}")
    print(f"  DARK CURRENT FITTING — Implicit Diode Model")
    print(f"{'='*50}")
    print(f"  T = {T} K, Vt = {Vt:.6f} V"
    ...
    print(f"\n{'='*50}")
    print(f"  DONE — {len(all_results)} model(s) fitted")
    print(f"{'='*50}")
...
if __name__ == '__main__':
    ...
    p = argparse.ArgumentParser(
        description='Dark Current Fitting — Implicit Single/Dual Diode Model')
    p.add_argument('data', help='Path to input data file (.txt)')
    p.add_argument('-o', '--output', default=None, help='Output directory')
    p.add_argument('--model', choices=['single', 'dual', 'both'],
                   default='dual', help='Diode model (default: dual)')
